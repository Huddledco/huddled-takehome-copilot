...
function getTimestamp() {
  ...
  const startDate = new Date("2022-05-01T00:00:00Z").getTime();
  const endDate = new Date("2022-06-01T23:59:59Z").getTime();
  return Math.floor(Math.random() * (endDate - startDate + 1)) + startDate;
}

function getRandomUserInteraction(tracks) {
...
function seedUsers(amount) {
  ...
  users.forEach((user) => query.run(user.username, user.timezone));

  console.log("Users table seeded successfully.");
}

function seedUserSessions(amount) {
  ...
  // Get all user ids
  const userIdsQuery = db.prepare("SELECT id FROM users");
  const userIds = userIdsQuery.all();
  ...
  db.transaction(() => {
    userSessions.forEach(({ user_id, created_at, active }) => {
...
function seedArtists(amount) {
  ...
  const artists = Array.from({ length: amount }, (_, i) => ({
    name: `Artist ${i + 1}`,
  }));
  const query = db.prepare(
    "INSERT INTO artists (name) VALUES (?)"
  );
  artists.forEach((artist) => query.run(artist.name));

  console.log("Artists table seeded successfully.");
}

function seedTracks(amount) {
  ...
  const tracks = Array.from({ length: amount }, (_, i) => ({
    title: `Track ${i + 1}`,
  ...
  db.transaction(() => {
    tracks.forEach(({ title, artist_id }) => {
...
function seedVisits(amount) {
  ...
  // Get all artist ids
  const artistIdsQuery = db.prepare("SELECT id FROM artists");
  const artistIds = artistIdsQuery.all();

  // Get all sessions
  const sessionsQuery = db.prepare("SELECT id FROM sessions");
  const sessions = sessionsQuery.all();

  // Generate mock data for visits
  const visits = Array.from({ length: amount }, () => {
  ...
  db.transaction(() => {
    visits.forEach(({ artist_id, session_id, start_time, end_time }) => {
...
function seedUserEvents(amount) {
  ...
  // Get all tracks
  const tracksQuery = db.prepare("SELECT id FROM tracks");
  const tracks = tracksQuery.all();

  // Seed the user_events table
  const userEvents = Array.from({ length: amount }, () => {
